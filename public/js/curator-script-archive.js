...
searchCloseButton.onclick = function(){
  editingSource = false;
  addSourceButton.style.display = 'block';
  sourceSearchPopup.style.display = 'none';
...
let importButtonOnclick = function(source){
    return function(){
    ...
    var newImg = document.createElement('img');
    newImg.setAttribute('src', source.image);
    newBubble.appendChild(newImg);
    ...
    newButton.onclick = function(){
        if(waiting === false){
            filterPopup.style.display = 'table';
        }
    }
    ...
    newBubbleDeleteIcon.onclick = function(){
    ...
    newBubbleEditIcon.onclick = function(){
        editingSource = true;
        editSourceIndex = Array.from(this.parentNode.parentNode.parentNode.children).indexOf(this.parentNode.parentNode);
        choicesContainer.style.display = 'block';
        sourceSearchPopup.style.display = 'table';
    }
    ...
    newBubble.style.display = 'block';
    sourceBubbleContainer.insertBefore(newBubble, addSourceButton);
    ...
    sourceBubbles = document.getElementsByClassName('bubble');

    function waitForTracks(){
         if(typeof source.sourceSearch.source !== "undefined"){

            newButton.innerHTML = '';
            newButton.innerHTML = '<p>Filter<p>';
            newButton.style.cursor = 'pointer';

            newButton.onmouseover = function(){
            newButton.style.backgroundColor = 'green';
            newButton.firstChild.style.color = 'white';
            newButton.style.cursor = 'pointer';
            }
            ...
            filterTracklistHeader.appendChild(newImg);
    
            var newInfo = document.createElement('div');
            newInfo.className = 'tracklist-info';
    
            var newTitle = document.createElement('h4');
            newTitle.className = 'tracklist-title';
            newTitle.innerHTML = source.name;
    
            newInfo.appendChild(newTitle);
    
            var totalTracks = document.createElement('p');
            totalTracks.className = 'data';
            totalTracks.innerHTML = 'Total Tracks: ' + source.tracks.length;
    
            var filteredTracks = document.createElement('p');
            filteredTracks.className = 'data';
            filteredTracks.innerHTML = 'Filtered Tracks: ' + source.tracks.length;
    
            newInfo.appendChild(totalTracks);
            newInfo.appendChild(filteredTracks);
    
            filterTracklistHeader.appendChild(newInfo);
            ...
            for(let i=0; i<source.tracks.length; i++){
                ...
                newTrack.className = 'track';
                var newTrackImg = document.createElement('img');
                newTrackImg.src = source.tracks[i].image;
                var newTrackName = document.createElement('p');
                newTrackName.className = 'name';
                newTrackName.innerHTML = source.tracks[i].name;
                var newTrackArtists = document.createElement('p');
                newTrackArtists.className = 'artists';
                newTrackArtists.innerHTML = source.tracks[i].getArtistString();
                var newTrackInfo = document.createElement('div');
                newTrackInfo.className = 'track-info';
                ...
                newTrackInfo.appendChild(newTrackArtists);
                newTrack.appendChild(newTrackInfo);
                filterTracklist.appendChild(newTrack);
            }
            waiting = false;
...
//Source Search Options Onclick
sourceSearchChoices[0].onclick = function(){
    if(editingSource){
        choicesContainer.style.display = 'none';
        searcherContainer.style.display = 'block';

        var source = new Source('playlist', _token);

        searcherContainer.appendChild(source.sourceSearch.mainContainer);
        var importButton = document.querySelector('.source-search > .yes');
        importButton.onclick = function(){
            sourceSearchPopup.style.display = 'none';
            sourceBubbles[editSourceIndex].children[1].children[0].innerHTML = source.name;
            sourceBubbles[editSourceIndex].children[1].children[1].innerHTML = source.owner;
            sourceBubbles[editSourceIndex].children[2].innerHTML = '<p>Loading...</p>';
            sourceArray[editSourceIndex] = source;
            editingSource = false;
            searcherContainer.innerHTML='';
        }
    }
        ...
        searcherContainer.appendChild(source.sourceSearch.mainContainer);
        var importButton = document.querySelector('.source-search > .yes');
        importButton.onclick = importButtonOnclick(source);
    }
}
sourceSearchChoices[1].onclick = function(){
    if(editingSource){
        choicesContainer.style.display = 'none';
        searcherContainer.style.display = 'block';

        var source = new Source('album', _token);

        searcherContainer.appendChild(source.sourceSearch.mainContainer);
        var importButton = document.querySelector('.source-search > .yes');
        importButton.onclick = function(){
            sourceSearchPopup.style.display = 'none';
            sourceBubbles[editSourceIndex].children[0].src = source.image;
            sourceBubbles[editSourceIndex].children[1].children[0].innerHTML = source.name;
            sourceBubbles[editSourceIndex].children[1].children[1].innerHTML = source.owner;
            sourceArray[editSourceIndex] = source;
            editingSource = false;
...
sourceSearchChoices[2].onclick = function(){
    if(editingSource){
        choicesContainer.style.display = 'none';
        searcherContainer.style.display = 'block';

        var source = new Source('track', _token);
